...
import { Check, Crown, Sparkles, Zap, Calendar, Utensils, Camera, Loader2 } from 'lucide-react';
...
export function Paywall({ onClose, redirectAfterTrial = '/home' }: PaywallProps) {
  ...
  const { startTrial, redirectToStripeCheckout, checkoutLoading } = useSubscription();
  ...
  const handleSubscribe = async () => {
    ...
    try {
      // In beta mode, use the mock trial start
      if (BETA_MODE_ENABLED) {
        ...
        if (success) {
          ...
          throw new Error('Failed to start access');
        }
      } else {
        // Production: Redirect to Stripe Checkout
        trackEvent('checkout_started', { plan_type: selectedPlan });
        await redirectToStripeCheckout(selectedPlan);
        // Note: User will be redirected to Stripe, so no need to handle success here
      }
    } catch (error) {
      toast({
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
      <div className="h-screen overflow-y-auto">
        <div className="max-w-md mx-auto px-4 py-8 pb-20">
                ...
                <CardContent className="p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <p className="text-sm font-bold text-green-600 dark:text-green-400">🎉 BETA ACCESS</p>
                      <p className="text-xs text-muted-foreground">Enjoy free premium access during beta!</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
          ...
          {/* Promo Banner - Only show in production */}
          {!BETA_MODE_ENABLED && (
            <div className="mb-6 animate-fade-in">
              <Card className="border-2 border-primary bg-gradient-to-r from-primary/10 via-primary/5 to-primary/10">
                <CardContent className="p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <p className="text-sm font-bold text-foreground">🎁 LAUNCH SPECIAL</p>
                      <p className="text-xs text-muted-foreground">Pay annually & save 40%!</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
          ...
          <div className="mb-8 text-center">
            ...
            <h1 className="text-3xl font-bold text-foreground">
              {BETA_MODE_ENABLED ? 'Get Premium Access' : 'Choose Your Plan'}
            </h1>
          ...
          <div className="mt-4 space-y-2 text-center">
            {BETA_MODE_ENABLED ? (
              <p className="text-sm text-muted-foreground">
                <span className="font-semibold text-green-600">Free during beta!</span> No credit card required.
              </p>
            ) : (
              <>
                <p className="text-sm text-muted-foreground">
